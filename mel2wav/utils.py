...
def get_magnitude_melspec(wave: torch.Tensor, n_fft: int):
    ...
    """
    window = torch.hann_window(n_fft).float().cuda()
    stft = torch.stft(
        wave, n_fft=n_fft, hop_length=n_fft // 4, win_length=n_fft,
        window=window, center=True, return_complex=True, pad_mode="constant")
    ...
    for n_fft in n_ffts:
        ...
        loss = loss + F.l1_loss(
            spec_1, spec_2, reduction="mean") + F.l1_loss(
                torch.log(spec_1 + 1e-3), torch.log(spec_2 + 1e-3), reduction="mean")
